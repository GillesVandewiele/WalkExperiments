...
from graph_no_hashing import rdflib_to_kg
from rdf2vec_no_hashing import RDF2VecTransformer
...
g = rdflib.Graph()
g.parse('../data/MUTAG/mutag.xml')
print('OK')

# Load our train & test instances and labels
test_data = pd.read_csv('../data/MUTAG/MUTAG_test.tsv', sep='\t')
train_data = pd.read_csv('../data/MUTAG/MUTAG_train.tsv', sep='\t')
...
transformer = RDF2VecTransformer(wl=False, max_path_depth=2, sg=1)
walk_embeddings = transformer.fit_transform(kg, train_people + test_people)

# Create embeddings using Weisfeiler-Lehman
transformer = RDF2VecTransformer(wl=True, max_path_depth=2, sg=1)
...
ax[0].set_title('Random Walk Embeddings')
ax[1].set_title('Weisfeiler-Lehman Embeddings')
plt.savefig('identical.svg', format='svg')
plt.show()